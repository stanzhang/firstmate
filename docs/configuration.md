...
`data/` holds durable private fleet records such as the project and secondmate registries, captain preferences, optional shared captain preferences, learnings, backlog, briefs, scout reports, and outward consultation evidence under `data/consultations/`.
[`docs/consultation.md`](consultation.md) owns the outward consultation lifecycle, while `bin/fm-consult.mjs` owns its exact child-artifact mechanics.
